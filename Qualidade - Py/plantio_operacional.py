...
caminho_completo = os.path.abspath(
    r"\\GLWFS02.lwart.net\LWC-FLORESTAL\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\QLDplantiooperacional_attachments")
...
username = cred['login']
password = cred['senha']
survey_item = cred['QLD_plantio_operacional']
...
related_items = survey_by_id.related_items('Survey2Service', 'forward')
if related_items:
    feature_service_item = related_items[0]
    # Assuming you're interested in the first layer
    feature_service = feature_service_item.layers[0]
else:
    print("No related Survey2Service found.")
    exit()
...
#    if str(row['regiao']) == "MS" and str(row['fazenda'])[0] == '2':
#        base.at[index, "fazenda"] = str("6" + str(row['fazenda'])[-3:])
#    else:
#        base.at[index, "fazenda"] = str(row['fazenda'])
