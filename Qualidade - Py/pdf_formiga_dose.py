...
path = r"\\GLWFS02.lwart.net\LWC-FLORESTAL\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\FormigaManualDosagem_attachments"
...
base_fotos_balanca = base_fotos_balanca.rename(
    columns={"tipo": "tipo_evidencia", 'url': 'balanca'})
...
url_base_survey = r'F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\QLD_formiga_manual_dosagem.xlsx'
...
survey = survey.merge(base_fotos_balanca[[
                      'balanca', 'tipo_evidencia', 'objectid']], left_on='objectid', right_on='objectid', how='left')
...
survey['url_coleta'] = survey['url_coleta'].apply(
    lambda x: foto_branco if pd.isnull(x) else x)
survey['balanca'] = survey['balanca'].apply(
    lambda x: foto_branco if pd.isnull(x) else x)
...
caminho_completo = os.path.abspath(
    r"F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\Arquivos PDF\Formiga Manual Dosagem")
...
for i, row in survey.iterrows():
    try:
        ...
        cnv = canvas.Canvas(
            r'F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\Arquivos PDF\Formiga Manual Dosagem/' + filename)
