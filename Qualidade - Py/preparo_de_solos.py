...
caminho_completo = os.path.abspath(
    r"F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\QLDpreparodesolo_attachments")
...
username = cred['login']
password = cred['senha']
survey_item_id = cred['QLD_preparo_de_solo']
...
#    if str(row['regiao']) == "MS" and str(row['nome_fazenda'])[0] == '2':
#        base.at[index, "nome_fazenda"] = str(
#            "6" + str(row['nome_fazenda'])[-3:])
#    else:
#        base.at[index, "nome_fazenda"] = str(row['nome_fazenda'])
