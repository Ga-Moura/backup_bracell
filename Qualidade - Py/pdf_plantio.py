...
path = r"F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\QLDplantiooperacional_attachments"
...
base_fotos_evidencia_nc = base_fotos_evidencia_nc.rename(
    columns={"tipo": "tipo_evidencia_avaliacao_nc", 'url': 'url_evidencia_avaliacao_nc'})
...
base_fotos_viveiro = base_fotos.loc[base_fotos['tipo']
                                    == 'viveiro_espera_foto']
base_fotos_viveiro = base_fotos_viveiro.rename(
    columns={'tipo': 'tipo_viveiro_espera_foto', 'url': 'url_viveiro_espera_foto'})


# In[49]:


url_base_att = r'F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\QLD_plantio_operacional_attachments.csv'


# In[50]:


url_base_survey = r'F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\QLD_plantio_operacional.xlsx'
...
survey = survey.merge(base_fotos_evidencia_nc[[
                      'url_evidencia_avaliacao_nc', 'tipo_evidencia_avaliacao_nc', 'objectid']], left_on='objectid', right_on='objectid', how='left')


# In[60]:


survey = survey.merge(base_fotos_viveiro[['url_viveiro_espera_foto', 'tipo_viveiro_espera_foto',
...
survey['url_coleta'] = survey['url_coleta'].apply(
    lambda x: foto_branco if pd.isnull(x) else x)
survey['url_evidencia_avaliacao_nc'] = survey['url_evidencia_avaliacao_nc'].apply(
    lambda x: foto_branco if pd.isnull(x) else x)
survey['url_viveiro_espera_foto'] = survey['url_viveiro_espera_foto'].apply(
    lambda x: foto_branco if pd.isnull(x) else x)
...
caminho_completo = os.path.abspath(
    r"F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\Arquivos PDF\Plantio Operacional")
...
for i, row in survey.iterrows():
    try:
        ...
        cnv = canvas.Canvas(
            r'F:\Qualidade_Florestal\01- SÃO PAULO\02- Silvicultura e Sobrevivência\00 - Arquivos Surveys\Arquivos PDF\Plantio Operacional/' + filename)
    ...
    # Adicionar imagem aqui
        ...
        cnv.drawImage(str(row['url_evidencia_avaliacao_nc']),
                      40, 80, width=200, height=200)

    # Adicionar imagem aqui
        cnv.setFont('Helvetica', 13)
        cnv.drawString((360), (300), 'Viveiro de Espera')
        cnv.drawImage(str(row['url_viveiro_espera_foto']),
